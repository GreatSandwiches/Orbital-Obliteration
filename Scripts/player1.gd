...
var ship_vector = Vector2(0,0)
var ship_vector_forward = Vector2(0,0)
var ship_vector_backward = Vector2(0,0)
...
var knockback_amount = 0.0005
var missile_damage = 2.5
var mine_knockback = 0.07
var rapid_fire_rate = 0.1
var rapid_cooling_rate = 8
var low_health = 30
...
func _missile_powerup_collected():
	missile = 1 # Activates the missile (1 is active, 0 is inactive)
...
# Function to activate the shotgun when powerup collected
...
func _on_rapidfire_entered(area):
	if area.has_meta("rapidfire"):
		global.p1_firerate = rapid_fire_rate
		global.p1_coolingrate = rapid_cooling_rate
...
# Function to activate the damage powerup
...
# Deactivates the damage powerup when timer runs out
func _on_DamagePowerupTimer_timeout():
	big_bullet = false


# Timer controlling firerate
func _on_timer_timeout():
	can_shoot = true


# Function to allow player to shoot once gun is no longer overheated
...
func _process(delta):
	...
	# Updating global posiition and velocity constantly
	...
	if ship_vector.x >= 0:
		ship_vector_backward.x = 0.01 * (pow((ship_vector.x + 1.2), 3) - 1.728)
	if ship_vector.x <= 0:
		ship_vector_backward.x = 0.01 * (pow((ship_vector.x - 1.2), 3) + 1.728)
	if ship_vector.y >= 0:
		ship_vector_backward.y = 0.01 * (pow((ship_vector.y + 1.2), 3) - 1.728)
	if ship_vector.y <= 0:
		ship_vector_backward.y = 0.01 * (pow((ship_vector.y - 1.2), 3) + 1.728)
	
	# Current position identifier for use with trail
	global.p1_location = -10 * transform.x + position
	velocity = 400 * ship_vector
	
	# Smoke trail activation
	if global.p1_health < low_health:
